...
// .wrangler/tmp/bundle-9P86wI/checked-fetch.js
...
async function loadAsset(env, filename) {
  const key = assetManifest[filename];
  if (!key) {
    return `Asset not found: ${filename}`;
  }
  return await env.__STATIC_CONTENT.get(key);
}
...
var src_default = {
  async fetch(request, env) {
    ...
    switch (url.pathname) {
      ...
      case "/dashboard": {
        const html = await loadAsset(env, "dashboard.html");
...
// .wrangler/tmp/bundle-9P86wI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-9P86wI/middleware-loader.entry.ts
